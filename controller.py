...
class Controller:
    ...
    def _handle_wait_for_data(self):
        ...
        # PREVENT AUTO-DISARM BY SPAMMING ARM EVERY 1 SECOND WHILE WAITING ON THE GROUND
        if int(elapsed * 10) % 10 == 0:
            self.arm()

        if has_pos and track_received and race_started:
            self._load_track_data()
            self._change_state(STATE_TAKEOFF)
        elif elapsed > 10.0 and race_started:
